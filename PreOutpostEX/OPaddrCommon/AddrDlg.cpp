...
BEGIN_MESSAGE_MAP(AddrDlg, CDialogEx)
  ...
  ON_WM_PAINT()
  ON_WM_QUERYDRAGICON()
  ON_COMMAND(ID_ABOUT,        &AddrDlg::OnAbout)
  ON_COMMAND(ID_Refresh,      &AddrDlg::OnRefresh)
  ON_COMMAND(ID_NewAddress,   &AddrDlg::OnAddressNew)
  ON_COMMAND(ID_SaveAll,      &AddrDlg::OnSaveall)
  ON_COMMAND(ID_AddrDelete,   &AddrDlg::OnAddrdelete)
  ON_LBN_DBLCLK(IDC_LIST,     &AddrDlg::OnDblclkList)
  ON_COMMAND(ID_AddressEdit,  &AddrDlg::OnAddressEdit)
  ON_COMMAND(ID_ROUTING_FORM, &AddrDlg::OnRoutingForm)
  ON_COMMAND(ID_ROUTING_HELP, &AddrDlg::OnRoutingHelp)
  ON_WM_SIZE()
  ON_WM_MOVE()
  ON_WM_CLOSE()
END_MESSAGE_MAP()
...
void AddrDlg::OnRoutingForm() {
  showPDF(_T("RoutingSlip.pdf"));
  }


void AddrDlg::OnRoutingHelp() {
